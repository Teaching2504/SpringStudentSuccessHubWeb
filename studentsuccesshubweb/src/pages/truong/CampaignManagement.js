...
import { Plus, Sliders, History, Award, CheckCircle, AlertCircle, Edit2, Trash2, TrendingUp } from 'lucide-react';
...
const CampaignManagement = () => {
  ...
  return (
    <div className="space-y-6">
            ...
            <div key={c.maDot} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-5">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-slate-100">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button onClick={() => handleOpenRuleModal(c)} className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 text-xs font-semibold rounded-xl transition cursor-pointer">
                  ...
                  <button onClick={() => handleOpenCampModal(c)} className="p-2 border border-slate-200 rounded-xl hover:bg-slate-50 text-slate-600 cursor-pointer"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => handleDeleteCamp(c.maDot)} className="p-2 border border-rose-200 rounded-xl hover:bg-rose-50 text-rose-600 cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
      ...
      <Modal isOpen={isCampModalOpen} onClose={() => setIsCampModalOpen(false)} title={editingCamp ? 'Cập nhật đợt xét học bổng' : 'Khởi tạo đợt xét học bổng mới'}>
        ...
        <form onSubmit={handleSaveCamp} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
            <button type="button" onClick={() => setIsCampModalOpen(false)} className="px-4 py-2 border border-slate-300 text-slate-700 rounded-xl text-sm hover:bg-slate-50 transition cursor-pointer">Hủy</button>
      ...
      <Modal isOpen={isRuleModalOpen} onClose={() => setIsRuleModalOpen(false)} title={`Cấu hình Dynamic Rule Engine: ${selectedCampForRules?.tenDot}`}>
        <div className="space-y-6">
