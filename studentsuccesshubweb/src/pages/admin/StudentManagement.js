...
import { UserPlus, Search, Edit2, Trash2, AlertTriangle, CheckCircle, FileSpreadsheet, AlertCircle, Download } from 'lucide-react';
...
const StudentManagement = () => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-700">
            <thead className="bg-slate-50 border-b border-slate-200 text-xs font-semibold uppercase text-slate-500">
              <tr>
                <th className="px-4 py-3.5">MSSV</th><th className="px-4 py-3.5">Họ và Tên</th><th className="px-4 py-3.5">Lớp / Khoa</th>
                <th className="px-4 py-3.5 text-center">GPA</th><th className="px-4 py-3.5 text-center">ĐRL</th><th className="px-4 py-3.5 text-center">Tín chỉ</th>
                <th className="px-4 py-3.5">Cảnh báo Học vụ</th><th className="px-4 py-3.5 text-right">Thao tác</th>
              </tr>
            </thead>
                  ...
                  return (
                    <tr key={sv.mssv} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-right space-x-2">
                        <button onClick={() => handleOpenModal(sv)} className="p-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 cursor-pointer"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(sv.mssv)} className="p-1.5 rounded-lg border border-rose-200 text-rose-600 hover:bg-rose-50 cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                      </td>
                    </tr>
                  );
                })
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingStudent ? `Cập nhật sinh viên MSSV: ${editingStudent.mssv}` : 'Thêm hồ sơ sinh viên mới'}>
      ...
      <Modal isOpen={isImportModalOpen} onClose={() => setIsImportModalOpen(false)} title="Nhập dữ liệu Sinh viên & Điểm từ file Excel (.xlsx)">
        <form onSubmit={handleImportExcel} className="space-y-4">
          <div className="p-4 bg-emerald-50/80 border border-emerald-200 rounded-2xl space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                <div className="text-sm font-bold text-emerald-950 flex items-center gap-1.5"><FileSpreadsheet className="w-4 h-4 text-emerald-700" /> File Excel mẫu chuẩn (.xlsx)</div>
                <p className="text-xs text-emerald-800 mt-0.5">Tải file mẫu có sẵn dữ liệu mẫu & hướng dẫn chi tiết từng cột để nhập liệu.</p>
              </div>
              <button type="button" onClick={handleDownloadTemplate} className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-semibold rounded-xl shadow-sm transition cursor-pointer self-start sm:self-auto"><Download className="w-3.5 h-3.5" /> Tải mẫu .xlsx</button>
            </div>
            <div className="text-xs text-emerald-900 border-t border-emerald-200/80 pt-2.5">
              <span className="font-semibold text-emerald-950">Quy định 10 cột: </span>1. MSSV (*) | 2. Họ Tên (*) | 3. Email | 4. SĐT | 5. Mã Lớp (*) | 6. Giới tính | 7. GPA | 8. ĐRL | 9. Số TC | 10. Rớt môn (Khong/Co)
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Chọn Học kỳ áp dụng điểm</label>
            <select value={selectedHk} onChange={e => setSelectedHk(e.target.value)} className="w-full px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-xl text-sm font-medium">{hocKys.map(h => <option key={h.maHocKy} value={h.maHocKy}>{h.tenHocKy}</option>)}</select>
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Chọn file Excel</label>
            <input type="file" accept=".xlsx, .xls" onChange={e => setImportFile(e.target.files[0])} className="w-full px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-xl text-sm" />
          </div>
          {importResult && (
            <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-800 space-y-1">
              <p className="font-bold flex items-center gap-1"><CheckCircle className="w-4 h-4 text-emerald-600" /> Nhập dữ liệu thành công!</p>
              <p>• Thêm mới: {importResult.importedCount} sinh viên</p>
              <p>• Cập nhật điểm: {importResult.updatedCount} sinh viên</p>
              {importResult.errors && importResult.errors.length > 0 && (
                <div className="mt-2 text-rose-600"><p className="font-semibold">Cảnh báo dòng lỗi:</p><ul className="list-disc pl-4 space-y-0.5">{importResult.errors.map((err, i) => <li key={i}>{err}</li>)}</ul></div>
              )}
            </div>
          )}
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
            <button type="button" onClick={() => setIsImportModalOpen(false)} className="px-4 py-2 border border-slate-300 text-slate-700 rounded-xl text-sm hover:bg-slate-50 transition cursor-pointer">Đóng</button>
            <button type="submit" disabled={importLoading} className="px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-sm font-medium shadow-md shadow-emerald-700/20 transition cursor-pointer disabled:opacity-60">{importLoading ? 'Đang đọc file...' : 'Tiến hành Nhập & Đồng bộ'}</button>
          </div>
        </form>
      </Modal>
    </div>
