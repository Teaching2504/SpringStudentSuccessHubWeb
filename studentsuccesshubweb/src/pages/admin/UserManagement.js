...
import { UserPlus, Search, Edit2, Trash2, Lock, Unlock, AlertCircle, KeyRound, Eye, EyeOff, CheckCircle2 } from 'lucide-react';
...
const UserManagement = () => {
  ...
  const handleToggleStatus = async (user) => {
    ...
    const msg = isLocking ? `Bạn có chắc chắn muốn KHÓA tài khoản "${user.tenDangNhap}" (${user.hoTen}) không?` : `Bạn có muốn MỞ KHÓA cho tài khoản "${user.tenDangNhap}" (${user.hoTen}) không?`;
  ...
  return (
    <div className="space-y-6">
                ...
                filteredUsers.map(u => (
                  <tr key={u.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-right space-x-1.5 whitespace-nowrap">
                      <button onClick={() => handleOpenResetModal(u)} title="Đặt lại mật khẩu nhanh" className="p-1.5 rounded-lg border border-amber-200 text-amber-700 hover:bg-amber-50 text-xs cursor-pointer transition inline-flex items-center gap-1 font-medium">
                        <KeyRound className="w-3.5 h-3.5" /><span className="hidden md:inline text-[11px]">Đổi MK</span>
                      </button>
                      <button onClick={() => handleToggleStatus(u)} title={u.trangThai === 'HOAT_DONG' ? 'Khóa tài khoản' : 'Mở khóa tài khoản'} className={`p-1.5 rounded-lg border text-xs cursor-pointer transition ${u.trangThai === 'HOAT_DONG' ? 'text-slate-500 border-slate-200 hover:bg-slate-100' : 'text-emerald-600 border-emerald-200 hover:bg-emerald-50'}`}>
                        {u.trangThai === 'HOAT_DONG' ? <Lock className="w-3.5 h-3.5" /> : <Unlock className="w-3.5 h-3.5" />}
                      </button>
                      <button onClick={() => handleOpenModal(u)} title="Chỉnh sửa thông tin" className="p-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 text-xs cursor-pointer transition"><Edit2 className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDelete(u.id, u.tenDangNhap)} title="Xóa tài khoản" className="p-1.5 rounded-lg border border-rose-200 text-rose-600 hover:bg-rose-50 text-xs cursor-pointer transition"><Trash2 className="w-3.5 h-3.5" /></button>
                    </td>
                  </tr>
                ))
              )}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingUser ? `Cập nhật thông tin tài khoản (${editingUser.tenDangNhap})` : 'Tạo mới tài khoản người dùng'}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-semibold text-slate-700">{editingUser ? 'Mật khẩu mới (để trống nếu không đổi)' : 'Mật khẩu khởi tạo'}</label>
              <button type="button" onClick={() => setFormData({ ...formData, matKhau: '123456' })} className="text-[11px] text-primary-600 hover:underline cursor-pointer">Đặt mặc định (123456)</button>
            </div>
            <div className="relative">
              <input type={showPassword ? 'text' : 'password'} value={formData.matKhau} onChange={e => setFormData({ ...formData, matKhau: e.target.value })} placeholder={editingUser ? '••••••••' : 'Mặc định: 123456'} className="w-full px-3.5 py-2 pr-10 bg-slate-50 border border-slate-300 rounded-xl text-sm font-mono" />
            ...
            <p className="text-[11px] text-slate-400 mt-1">* Mật khẩu sẽ tự động mã hóa chuẩn BCrypt và cập nhật ngay lập tức.</p>
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-xs font-semibold text-slate-700 mb-1">Email</label><input type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} placeholder="user@ou.edu.vn" className="w-full px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-xl text-sm" /></div>
            <div><label className="block text-xs font-semibold text-slate-700 mb-1">Số điện thoại</label><input type="text" value={formData.soDienThoai} onChange={e => setFormData({ ...formData, soDienThoai: e.target.value })} placeholder="0901234567" className="w-full px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-xl text-sm" /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
            ...
            <button type="submit" className="px-4 py-2 bg-primary-700 hover:bg-primary-800 text-white rounded-xl text-sm font-medium shadow-md shadow-primary-700/20 transition cursor-pointer">{editingUser ? 'Lưu thay đổi' : 'Tạo tài khoản'}</button>
          </div>
        </form>
      </Modal>

      <Modal isOpen={isResetModalOpen} onClose={() => setIsResetModalOpen(false)} title={`Đặt lại mật khẩu cho "${selectedUserForReset?.tenDangNhap}"`}>
        {resetSuccess && <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-2 text-emerald-800 text-sm"><CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" /><span>{resetSuccess}</span></div>}
        {resetError && <div className="mb-4 p-3 bg-rose-50 border border-rose-200 rounded-xl flex items-center gap-2 text-rose-700 text-sm"><AlertCircle className="w-4 h-4 shrink-0" /><span>{resetError}</span></div>}
        <form onSubmit={handleExecuteResetPassword} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" required value={newResetPassword} onChange={e => setNewResetPassword(e.target.value)} placeholder="Nhập mật khẩu mới" className="w-full px-3.5 py-2 bg-white border border-slate-300 rounded-xl text-sm font-mono focus:ring-2 focus:ring-primary-500 focus:outline-none" />
            <div className="flex gap-2 mt-2">
              {['123456', 'Admin@123456'].map(pwd => (
                <button key={pwd} type="button" onClick={() => setNewResetPassword(pwd)} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs text-slate-700 cursor-pointer">{pwd}</button>
              ))}
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
            <button type="button" onClick={() => setIsResetModalOpen(false)} className="px-4 py-2 border border-slate-300 text-slate-700 rounded-xl text-sm hover:bg-slate-50 transition cursor-pointer">Đóng</button>
            <button type="submit" className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-sm font-medium shadow-md shadow-amber-600/20 transition cursor-pointer flex items-center gap-1.5"><KeyRound className="w-4 h-4" /> Xác nhận đổi mật khẩu</button>
          </div>
        </form>
      </Modal>
    </div>
