...
import { Award, Search, Download, Printer, BookOpen, RefreshCw } from 'lucide-react';
...
const FacultyScholarshipList = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Tìm MSSV, họ tên, lớp..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-primary-500 focus:outline-none" />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-semibold uppercase">
              <tr>
                <th className="px-4 py-3 text-center">Hạng</th><th className="px-4 py-3">Sinh viên</th><th className="px-4 py-3">Lớp & Ngành</th>
                <th className="px-4 py-3 text-center">GPA / ĐRL</th><th className="px-4 py-3 text-center">Tín chỉ</th><th className="px-4 py-3 text-center">Loại Học Bổng</th>
                <th className="px-4 py-3 text-right">Số Tiền (VNĐ)</th><th className="px-4 py-3 text-center">Trạng Thái</th><th className="px-4 py-3 text-center">Thao tác</th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                filteredList.map((item, idx) => (
                  <tr key={item.id || idx} className="hover:bg-slate-50/80 transition-colors">
                    <td className="px-4 py-3 text-center font-bold text-slate-800">{item.thuHang ? <span className="w-6 h-6 inline-flex items-center justify-center rounded-full bg-slate-100 text-slate-700 font-extrabold text-[11px]">{item.thuHang}</span> : '-'}</td>
                    ...
                    <td className="px-4 py-3 text-center"><Badge variant={item.trangThai === 'CHINH_THUC' || item.trangThai === 'DA_DUYET' ? 'emerald' : item.trangThai === 'DU_KIEN' ? 'blue' : 'slate'}>{item.trangThai === 'CHINH_THUC' || item.trangThai === 'DA_DUYET' ? 'Chính thức' : item.trangThai === 'DU_KIEN' ? 'Dự kiến' : item.trangThai || 'Chưa duyệt'}</Badge></td>
                    <td className="px-4 py-3 text-center">
                      <button type="button" onClick={() => handleViewGrades(item)} className="px-2.5 py-1 bg-primary-50 text-primary-700 hover:bg-primary-100 rounded-lg text-xs font-semibold transition cursor-pointer flex items-center gap-1 mx-auto">
      ...
      <Modal isOpen={showGradesModal} onClose={() => setShowGradesModal(false)} title={`Bảng điểm chi tiết: ${selectedStudentForModal?.hoTen || ''} (${selectedStudentForModal?.mssv || ''})`} maxWidth="max-w-3xl">
